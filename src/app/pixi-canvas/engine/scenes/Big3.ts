...
export class Big3 extends Scene {
    ...
    private animations: AnimatedEntity[] = [];
        ...
        if (isValid) {
            ...
            for (let i = 0; i < playedCards.length; i++) {
                ...
                this.animations.push(
                    card.animateSlideTo(650 + (i - middle) * 40 + 1, 350)
                );
            }
    ...
    updateDotColours() {
        for (let i = 0; i < 4; i++) {
            ...
            else if (this.big3Service.getCurrentTurn() == i) colour = Colours.Green;
            this.indicators[i]
                .rect(this.locations[i][0], this.locations[i][1], this.locations[i][2], this.locations[i][3])
                .fill(colour)
        }
    ...
    async waitForOpponentTurn() {
        ...
        const turn = this.big3Service.getNextTurn();
        this.updateDotColours();
    ...
    pushPlayedCards() {
        this.justPlayed.forEach(card => {
            this.animations.push(
                card.animateSlideTo(undefined, 200)
            )
        });
        this.lastPlayed.forEach(card => 
            {
                this.animations.push(
                    card.animateSlideTo(1000, -100, true)
                )
            }
        );
    ...
    update(delta: number) {
        ...
        this.animations = this.animations.filter(animation => animation.isAlive());
    }
}
